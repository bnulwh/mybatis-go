...
func change2UInt(val interface{}) (uint,error){
	...
	default:
		nv,err:= strconv.ParseUint(fmt.Sprintf("%v",val))
		if err != nil{
			return uint(0),err
		}
		return uint(nv),nil
...
func change2UInt64(val interface{}) (uint64,error){
	...
	default:
		return strconv.ParseUint(fmt.Sprintf("%v",val))
	}
}
func change2Float32(val interface{}) (float32,error){
	...
	default:
		fv,err:= strconv.ParseFloat(fmt.Sprintf("%v",val))
		if err != nil{
...
func change2Float64(val interface{}) (float64,error){
	...
	default:
		return strconv.ParseFloat(fmt.Sprintf("%v",val))
	}
}
