...
type OceanBaseOracleDialector struct {
	BaseDialector
}

func NewOceanBaseOracleDialector(cfg ConfigProvider) *OceanBaseOracleDialector {
	params := cfg.GetConnectParams()
	dsn := cfg.GetDSN()
	if dsn == "" {
		dsn = generateOracleDSN(params)
	}
	return &OceanBaseOracleDialector{
...
func (d *OceanBaseOracleDialector) NeedsReturning() bool {
	return false
}

func (d *OceanBaseOracleDialector) SystemTablePrefixes() []string {
...
func (d *OceanBaseOracleDialector) TableListSQL() string {
	schema := d.effectiveSchema()
