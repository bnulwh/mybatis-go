...
| 8.5 | 读写分离 | 主从路由 | ✅ 完全可行 | ✅ 已实现 | mybatis.replicas + context 路由，事务内强制主库 |
...
| 优先级 | 功能 | 工作量 | 影响 |
|--------|------|--------|------|
| P0-1 | **Wrapper 条件构造器** | 中 | 替代手写 WHERE，MP 最常用功能（`orm.NewQueryWrapper()` 链式 API：比较/模糊/区间/集合/Or/Nested/GroupBy/Having/OrderBy/Last/Select；自动注入——mapper 方法声明 `*QueryWrapper` 参数（可与 PageParam 组合），已有 WHERE 追加 AND、否则 WHERE，条件插在尾部子句之前，分页注入先于 COUNT；显式 `${ew}` 占位模式（Slots 含 ew 判定）；值内联走 sqlfragment.FormatValue，P0 限定 select，✅ 已完成） |
...
| P3-2 | **读写分离路由** | 中 | ✅ 已实现 | context 路由 + 事务强制主库 |
